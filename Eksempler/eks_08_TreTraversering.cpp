...
int main()  {
    ...
    treNivaa(root);
    cout << "\n\n Lengden på treet: " << maxNivaa << endl;
...
/*void traverserPostorder(Node* node) {
    ...
    while (!gStakk.empty() || node) {
        if (node) { // Gå så langt ned til venstre som mulig
            gStakk.push(node);
            node = node->left;
        } else {
            Node* toppNode = gStakk.top();
            if (!toppNode->right || toppNode->right == sisteBesokte) { // Hvis høyre barn er null eller allerede besøkt, besøk toppnoden
                besok(toppNode);
                gStakk.pop();
                sisteBesokte = toppNode;
            } else {
                node = toppNode->right; // Ellers, gå til høyre barn
            }
        }
    }
}
