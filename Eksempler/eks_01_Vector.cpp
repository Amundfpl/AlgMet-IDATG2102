...
//          for (int i = 0; i < antall; i++)  data[i] = 0;  //  Unødvendig?!!!
      ...
      bool insert(const int pos, const T t) {
          if (antall >= kapasitet)  {//  Sjekker om antallet er større enn lengden/kapasiteten til arrayen/vektoren
                resize(kapasitet + 100); //legger til 100 på kapasiteten til vektoren dersom antallet er større enn kapasiteten
          }
      ...
      void resize(const int nyLengde) { //  Øker (om mulig) arrayens lengde.
          ...
          if(nyLengde > kapasitet){
            T* nyData = new T[nyLengde]; //Alloker en ny array med den nye lengden

            for(int i = 0; i < antall; ++i){ //Kopierer den eksisterende dataen til den nye arrayen
                nyData[i] = data[i];
            }
            delete[] data; //Frigjør den gamle arrayen

            data = nyData; //Oppdaterer pekeren til å peke på den nye arrayen

            kapasitet = nyLengde; //Oppdaterer kapasiteten til den nye lengden
          }
          else {
            cout << "Ny lengde er ikke større en naaverende kapasitet... \n\n";
          }
      }
...
int main()  {
    ...
    iVec.push_front(20); iVec.push_front(30); iVec.push_front(40); iVec.push_back(50);
    iVec.push_front(20); iVec.push_front(30); iVec.push_front(40); iVec.push_back(50);
    iVec.display();
    cout << "Tester om det fungerer: " << iVec.capacity()
         << ",  Stoerrelse: " << iVec.size() << "\n\n\n";
    cVec.push_front(43); cVec.push_front(60); cVec.push_back(89);
    cout << "Forskjellige tegn til vektoren " << "med antall " << cVec.capacity() <<"\n\n";
    cVec.display();
    cout << "Skjer det noe? \n\n";

    cout << "Den gamle kapasiteten til i_vektoren " << iVec.capacity() <<"\n\n";
    iVec.resize(220);
    cout << "Naa er i_vektoren oppdatert til kapasiteten " << iVec.capacity() <<"\n\n";
    for(int i = 0; i<=321; i++){
        iVec.insert(i, i);
    }
    iVec.display();
